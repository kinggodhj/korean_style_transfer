...
export SOURCE_FILE=~/workspace/NMT/transformer/korean/data/news_chatbot/test
...
#acc
cd $ACC_PATH
x=$(python acc.py --path1 $name --path2 $class_model)
echo "file name:$name"
echo "Accuracy:$x"
...
x=$(ngram -ppl $name -lm newchat_3 -order 3 -debug 2)
